import { Memory, OrbLayout } from '../types';

export class LayoutManager {
  
  /**
   * Calculates the position and rotation for an Orb in Sphere Mode.
   */
  getSphereLayout(
    memory: Memory, 
    radius: number, 
    isHovered: boolean, 
    isDragging: boolean
  ): OrbLayout {
    // Spherical to Cartesian conversion
    const x = radius * Math.sin(memory.phi) * Math.cos(memory.theta);
    const y = radius * Math.cos(memory.phi);
    const z = radius * Math.sin(memory.phi) * Math.sin(memory.theta);

    const BASE_SCALE = memory.scale;
    const HOVER_SCALE_MULTIPLIER = 1.2;
    const activeScale = (isHovered || isDragging) ? BASE_SCALE * HOVER_SCALE_MULTIPLIER : BASE_SCALE;

    return {
      x,
      y,
      z,
      rotateX: 0, // In sphere mode, rotation is handled by the world container or local random jitter
      rotateY: 0,
      scale: activeScale,
      opacity: 1,
      zIndex: Math.floor(z) + 2000, // Depth sorting
      blur: 0,
      isActive: false
    };
  }

  /**
   * Calculates the position and rotation for an Orb in Gallery Mode (Spiral).
   */
  getGalleryLayout(
    index: number, 
    focusedIndex: number
  ): OrbLayout {
    const offset = index - focusedIndex;
    const GOLDEN_ANGLE = 2.39996; 
    const Z_SPACING = 800; 
    const SPIRAL_RADIUS = 380; 

    // Spiral Math
    const z = -offset * Z_SPACING;
    const theta = index * GOLDEN_ANGLE;
    const absX = Math.cos(theta) * SPIRAL_RADIUS;
    const absY = Math.sin(theta) * SPIRAL_RADIUS;

    // Calculate position relative to the focused item (which should be centered)
    const activeTheta = focusedIndex * GOLDEN_ANGLE;
    const activeAbsX = Math.cos(activeTheta) * SPIRAL_RADIUS;
    const activeAbsY = Math.sin(activeTheta) * SPIRAL_RADIUS;

    const x = absX - activeAbsX;
    const y = absY - activeAbsY;

    // Visual adjustments based on distance from focus
    const dist = Math.abs(offset);
    const scale = offset === 0 ? 1.4 : Math.max(0.6, 1.2 - dist * 0.1);
    
    let opacity = 1;
    if (offset < 0) {
       opacity = Math.max(0, 1 + offset * 0.4); 
    } else {
       opacity = Math.max(0, 1 - offset * 0.15); 
    }

    const blur = dist * 2; 
    const isActive = offset === 0;
    
    // Higher z-index for active item to ensure it's on top
    const zIndex = isActive ? 10000 : (1000 - blur * 10);

    return {
      x,
      y,
      z,
      rotateX: 0,
      rotateY: 0,
      scale,
      opacity,
      zIndex,
      blur,
      isActive,
      side: x > 0 ? 'right' : 'left'
    };
  }
}
